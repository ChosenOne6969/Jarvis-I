...
class TibetanAudioSystem {
  ...
  scheduleBowlLoop(freqs, interval, sustain) {
    ...
    freqs.forEach((freq, i) => {
      this.createBowlStrike(freq, ctx.currentTime + i * (interval / freqs.length), sustain);
    });
    const timeout = setTimeout(() => {
      if (this.playing) this.scheduleBowlLoop(freqs, interval, sustain);
    }, interval * 1000);
...
function createNeuralNode(colorHex, glowLevel, progress) {
  ...
  for (let i = 0; i < numArms; i++) {
    ...
    const armMat = new THREE.LineBasicMaterial({
      color: c, transparent: true, opacity: intensity * (0.6 - i / numArms * 0.3)
    });
    group.add(new THREE.Line(
      new THREE.BufferGeometry().setFromPoints(new THREE.CatmullRomCurve3(pts).getPoints(12)),
      armMat
    ));
  ...
  if (progress > 0) {
    ...
    const ring = new THREE.Mesh(
      new THREE.TorusGeometry(0.18, 0.009, 4, 32, (progress / 100) * Math.PI * 2), ringMat
    );
...
export default function MindSpace() {
  ...
  useEffect(() => {
    ...
    scene.add(new THREE.Points(starGeo, new THREE.PointsMaterial({
      color: 0xd4b870, size: 0.022, transparent: true, opacity: 0.3
    })));
    ...
    fetch('/brain/brain_data.json').then(r => r.json()).then(data => {
      ...
      // Surface dots
      const surfDotsMat = new THREE.PointsMaterial({
        color: 0xffd060, size: 0.055,
        transparent: true, opacity: 0.88, sizeAttenuation: true,
      });
      brainGroup.add(new THREE.Points(
        new THREE.BufferGeometry().setFromPoints(surfPts), surfDotsMat
      ));
      animDataRef.current.surfDotsMat = surfDotsMat;

      // Surface connections — curved organic lines
      const surfShortMats = [];
      const surfLongPositions = [];
      ...
      connections.forEach(([i, j]) => {
        ...
        if (d < 0.85) {
          const midRaw = new THREE.Vector3(
          ...
          const mat = new THREE.LineBasicMaterial({
            color: 0xd4a830, transparent: true, opacity: baseOp,
          });
          brainGroup.add(new THREE.Line(geo, mat));
          surfShortMats.push({ mat, base: baseOp });
        } else {
          surfLongPositions.push(...a.toArray(), ...b.toArray());
        }
      });
      ...
      if (surfLongPositions.length > 0) {
        ...
        const longMat = new THREE.LineBasicMaterial({
          color: 0xb08818, transparent: true, opacity: 0.08,
        });
        brainGroup.add(new THREE.LineSegments(longGeo, longMat));
        animDataRef.current.surfLongMat = longMat;
      }

      // Internal dots — sparse
      if (intPts.length > 0) {
        const intDotsMat = new THREE.PointsMaterial({
          color: 0xffaa20, size: 0.042,
          transparent: true, opacity: 0.55, sizeAttenuation: true,
        });
        brainGroup.add(new THREE.Points(
          new THREE.BufferGeometry().setFromPoints(intPts), intDotsMat
        ));
        animDataRef.current.intDotsMat = intDotsMat;
      }

      // ── INTERNAL SYNAPSES ──
      // Sparse, properly connected, no clutter
      const synLines = [];
      ...
      for (let i = 0; i < intPts.length && synLines.length < 80; i++) {
        ...
        // Connect to 2 nearest only — keeps it clean
        let count = 0;
        for (const { d, j } of dists) {
          if (count >= 2) break;
          if (d > 0.65) break; // max connection distance
          ...
          const mat = new THREE.LineBasicMaterial({
            color: 0xffd060, transparent: true, opacity: baseOp,
          });
          brainGroup.add(new THREE.Line(geo, mat));
      ...
      setBrainLoaded(true);
    }).catch(err => { console.error('Brain load error:', err); setBrainLoaded(true); });
    ...
    animDataRef.current.pl1 = pl1; animDataRef.current.pl2 = pl2;
    animDataRef.current.insideLight = insideLight;
    ...
    const animate = () => {
      ...
      // Surface dots breathing
      if (ad.surfDotsMat) ad.surfDotsMat.opacity = inside
        ? 0.5 + 0.15 * Math.sin(t * 0.35)
        : 0.82 + 0.1 * Math.sin(t * 0.35);

      // Surface curves breathing
      if (ad.surfShortMats) ad.surfShortMats.forEach((s, i) => {
        s.mat.opacity = s.base * (0.75 + 0.25 * Math.sin(t * 0.28 + i * 0.04));
      });
      ...
      // Internal dots
      if (ad.intDotsMat) ad.intDotsMat.opacity = inside
        ? 0.65 + 0.22 * Math.sin(t * 0.4)
        : 0.42 + 0.15 * Math.sin(t * 0.4);

      // Synapses — sparse firing
      ...
      // Bloom
      if (ad.bloom) ad.bloom.strength = inside
        ? 1.6 + 0.2 * Math.sin(t * 0.3)
        : 1.0 + 0.12 * Math.sin(t * 0.3);

      if (ad.insideLight) ad.insideLight.intensity = inside
        ? 0.5 + 0.25 * Math.sin(t * 0.4) : 0;
      ...
      nodeGroupsRef.current.forEach((group, i) => {
        ...
        if (group.userData.coreMat) {
          const c = group.userData.color.clone();
          const dim = 0.4 + glow * 0.6;
          group.userData.coreMat.color.setRGB(c.r * dim, c.g * dim, c.b * dim);
        }
        ...
        if (group.userData.light) {
          group.userData.light.intensity = (inside ? 2.2 : 1.2) * glow * (0.6 + 0.4 * Math.sin(t * 1.4 + i));
        }
      });
  ...
  useEffect(() => {
    ...
    nodeGroupsRef.current.forEach(g => {
      if (g.userData.light) bg.remove(g.userData.light);
      bg.remove(g);
    });
  ...
  const enterBrain = () => {
    ...
    setIsTransitioning(true); setShowCreateMenu(false); setShowPanel(false);
  ...
  const exitBrain = () => {
    if (isTransitioning || !insideRef.current) return;
    setIsTransitioning(true); setShowPanel(false);
    const ctrl = controlsRef.current;
  ...
  const handleMouseMove = (e) => {
    if (Math.abs(e.clientX - mouseDownPos.current.x) > 4 ||
    ...
    if (hits.length > 0) {
      ...
      if (obj?.userData.nodeIndex !== undefined) {
        ...
        if (node) {
          ...
          setShowPanel(true); setShowSummary(false);
    ...
    if (clickCount.current === 1) {
      clickTimer.current = setTimeout(() => {
        ...
        if (!insideRef.current) {
          setShowPanel(false); setSelectedNode(null);
          setCreatePos({ x: e.clientX, y: e.clientY });
  ...
  const markVisited = async (id) => {
    try { await axios.patch(`${API}/nodes/${id}/visit`, {}, { headers }); } catch { }
  };
  ...
  const createNodeAt = async () => {
    ...
    try {
      const res = await axios.post(`${API}/nodes`, {
        title: 'UNNAMED', goal: '', milestones: [], resources: [], position: { x: 0, y: 0 }
      }, { headers });
      const newNode = {
        ...res.data,
        x3d: r * 2.0 * Math.sin(phi) * Math.cos(theta),
      ...
      setSelectedNode({ ...newNode }); setActiveMilestoneIdx(0);
      setActiveTab('milestones'); setShowPanel(true);
    } catch (err) { console.error(err); }
  };
  ...
  const addMilestone = () => {
    ...
    const milestones = [...(selectedNode.milestones || []), {
      title: newMilestoneTitle.trim(), description: '', order: (selectedNode.milestones || []).length, tasks: []
    }];
  ...
  const addTask = () => {
    ...
    const task = {
      text: newTaskText.trim(), difficulty: newTaskDifficulty, done: false,
    ...
    milestones[activeMilestoneIdx] = {
      ...milestones[activeMilestoneIdx],
      tasks: [...(milestones[activeMilestoneIdx].tasks || []), task]
    };
    setSelectedNode({ ...selectedNode, milestones, progress: calcProgress(milestones) });
  ...
  const suggestWithJarvis = async () => {
    ...
    try {
      const res = await axios.post(`${API}/jarvis/suggest`, {
        goal: selectedNode.goal, nodeTitle: selectedNode.title
      }, { headers });
      if (res.data.milestones?.length > 0) {
        const newMilestones = res.data.milestones.map((m, i) => ({
          ...
          tasks: (m.tasks || []).map(t => ({
            text: t.text, difficulty: t.difficulty || 'medium',
            done: false, deadline: null, createdAt: new Date().toISOString(),
            shouldDoFirst: t.shouldDoFirst || false,
          })),
        }));
  ...
  const saveNode = async () => {
    if (nodeOpenTime.current) {
      ...
      if (mins > 0) {
        try { await axios.patch(`${API}/nodes/${selectedNode._id}/visit`, { timeSpentMinutes: mins }, { headers }); } catch { }
      }
      nodeOpenTime.current = null;
    }
    try {
      const res = await axios.put(`${API}/nodes/${selectedNode._id}`, {
        title: selectedNode.title, goal: selectedNode.goal,
        milestones: selectedNode.milestones, resources: selectedNode.resources,
      }, { headers });
      const updated = nodesRef.current.map((n, i) =>
        n._id === selectedNode._id ? makeNode3D(res.data, i, nodesRef.current.length) : n
      );
  ...
  const overdueTasks = selectedNode?.milestones?.reduce((a, m) =>
    a + (m.tasks?.filter(t => !t.done && t.deadline && new Date(t.deadline) < new Date()).length || 0), 0) || 0;
  ...
  return (
    <div style={s.container}>
      ...
      <div style={s.hint}>
        {isInsideBrain ? 'INSIDE YOUR MIND · DRAG TO LOOK · CLICK NODES TO OPEN'
        ...
        <div style={s.panel}>
          ...
          <div style={s.progressSection}>
            <div style={s.progressHeader}>
              ...
              <span style={{ ...s.progressPct, color: progressColor(selectedNode.progress) }}>
          ...
          <div style={s.tabs}>
            <button style={{ ...s.tab, ...(activeTab === 'milestones' ? s.tabActive : {}) }}
              onClick={() => setActiveTab('milestones')}>
              MILESTONES{selectedNode.milestones?.length > 0 ? ` (${selectedNode.milestones.length})` : ''}
            </button>
            <button style={{ ...s.tab, ...(activeTab === 'resources' ? s.tabActive : {}) }}
          ...
          {activeTab === 'milestones' && (
            <div style={s.tabContent}>
              ...
              {showAddMilestone && (
                <div style={s.addForm}>
                  <input style={s.addInput} value={newMilestoneTitle}
                    placeholder="e.g. Foundation, Practice, Mastery..."
                ...
                <>
                  <div style={s.milestoneTabs}>
                    {selectedNode.milestones.map((m, i) => {
                      ...
                      return (
                        <button key={i}
                          style={{ ...s.milestoneTab, ...(i === activeMilestoneIdx ? s.milestoneTabActive : {}) }}
                    ...
                    <div style={s.milestoneBox}>
                      ...
                      <div style={s.taskList}>
                        {(activeMilestone.tasks || []).length === 0 && (
                          <p style={s.noTasksText}>No tasks yet — add below or ask JARVIS.</p>
                        )}
                        {(activeMilestone.tasks || []).map((task, ti) => {
                          ...
                          return (
                            <div key={ti} style={{ ...s.taskRow, ...(isOverdue ? s.taskRowOverdue : {}), ...(task.done ? s.taskRowDone : {}) }}>
                              <div style={{ ...s.taskCheck, ...(task.done ? s.taskCheckDone : {}) }}
                                onClick={() => toggleTask(activeMilestoneIdx, ti)}>
                                {task.done ? '✓' : ''}
                              </div>
                      ...
                      {showAddTask ? (
                        <div style={s.addTaskForm}>
                          <input style={s.addInput} value={newTaskText}
                            placeholder="Describe this task clearly..."
                            onChange={e => setNewTaskText(e.target.value)} autoFocus />
                          <div style={s.taskFormRow}>
                            <select style={s.diffSelect} value={newTaskDifficulty}
                            ...
                            <input type="date" style={s.dateInput} value={newTaskDeadline}
          ...
          {activeTab === 'resources' && (
            <div style={s.tabContent}>
              ...
              <div style={s.addForm}>
                <input style={s.addInput} value={newResource}
                  placeholder="Paste link or describe resource..."
      ...
      {jarvisOpen && (
        <div style={s.jarvis}>
          <p style={s.jarvisTitle}>◈ JARVIS INTERFACE</p>
...
const s = {
  ...
  hint: { position: 'fixed', top: '60px', left: '50%', transform: 'translateX(-50%)', color: `${gold}0.25)`, fontSize: '9px', letterSpacing: '3px', zIndex: 5, whiteSpace: 'nowrap' },
